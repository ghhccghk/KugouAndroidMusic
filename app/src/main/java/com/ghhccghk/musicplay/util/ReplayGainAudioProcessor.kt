...
class ReplayGainAudioProcessor : BaseAudioProcessor() {
	...
	fun setRootFormat(inputFormat: Format) {
		tags = ReplayGainUtil.Companion.parse(inputFormat)
	}
	...
	private fun computeGain() {
		...
		val gain = ReplayGainUtil.Companion.calculateGain(tags, mode, rgGain,
			reduceGain, ReplayGainUtil.Companion.RATIO)
		this.gain = gain?.first ?: ReplayGainUtil.Companion.dbToAmpl(nonRgGain.toFloat())
		...
		if (kneeThresholdDb != null) {
			...
			compressor!!.init(
				...
				ReplayGainUtil.Companion.TAU_ATTACK, ReplayGainUtil.Companion.TAU_RELEASE,
				ReplayGainUtil.Companion.RATIO
			)
		} else {
