...
typedef bool(*ZN7android18ExtendedMediaUtils26AudioTrackIsTrackOffloadedEi_t)(void *thisptr,
                                                                              uint32_t output);
...
typedef int32_t(*ZN7android11AudioEffect10getConfigsEP17audio_config_baseS2__t)(void *thisptr,
                                                                                void *c1, void *c2);
...
typedef int32_t(*ZN7android11AudioEffect19getEffectDescriptorEPK12audio_uuid_sP19effect_descriptor_s_t)(
        void *uuid, void *out);
...
typedef int32_t(*ZN7android11AudioEffect19getEffectDescriptorEPK12audio_uuid_sS3_jP19effect_descriptor_s_t)(
        void *uuid, void *type, uint32_t preferredTypeFlag, void *out);
...
typedef int32_t(*ZN7android11AudioSystem15getStreamVolumeE19audio_stream_type_tPfi_t)(int stream,
                                                                                      float *out,
                                                                                      int output);
...
bool initLib(JNIEnv *env) {
    ...
    android_namespace_t *ns = android_create_namespace_escape("default_copy", nullptr, nullptr,
                                                              ...
                                                              nullptr, nullptr);
    if (!libaudioclient_handle) {
        libaudioclient_handle = linkernsbypass_namespace_dlopen("libaudioclient.so", RTLD_GLOBAL,
                                                                ns);
        if (libaudioclient_handle == nullptr) {
    ...
    if (android_get_device_api_level() >= 31 && !libandroid_runtime_handle) {
        libandroid_runtime_handle = linkernsbypass_namespace_dlopen("libandroid_runtime.so",
                                                                    RTLD_GLOBAL, ns);
...
Java_org_nift4_gramophone_hificore_AudioTrackHiddenApi_dumpInternal(
    ...
    if (!initLib(env))
        return nullptr;
    DLSYM_OR_RETURN(libaudioclient, ZNK7android10AudioTrack4dumpEiRKNS_6VectorINS_8String16EEE,
                    nullptr)
    int pipe_fds[2];
    if (pipe(pipe_fds) == -1) {
...
#undef BUFFER_SIZE
        ...
        if (oreo) {
            for (auto port: *(std::vector<audio_port_oreo> *) nPorts) {
                ...
                if (port.id == id && port.active_config.ext.mix.handle == io &&
                    port.ext.mix.handle == io) {
...
extern "C"
JNIEXPORT jint JNICALL
Java_org_nift4_gramophone_hificore_ReflectionAudioEffect_00024Companion_getEffectConfigs(
        JNIEnv *env, jobject,
    ...
    int32_t ret = ZN7android11AudioEffect10getConfigsEP17audio_config_baseS2_((void *) ptr, &c1,
                                                                              &c2);
...
Java_org_nift4_gramophone_hificore_AudioSystemHiddenApi_getStreamVolumeInternal(JNIEnv *env,
    ...
    DLSYM_OR_ELSE(libaudioclient,
                  ZN7android11AudioSystem15getStreamVolumeE19audio_stream_type_tPfi) {
        env->ThrowNew(env->FindClass("java/lang/Exception"), "dlsym failed");
        return 0;
    }
    float out;
    int ret = ZN7android11AudioSystem15getStreamVolumeE19audio_stream_type_tPfi(stream, &out,
                                                                                output);
    if (ret != 0) {
        char buf[40];
...
Java_org_nift4_gramophone_hificore_ReflectionAudioEffect_00024Companion_getFlagsInternal(
        JNIEnv *env, jobject thiz, jlong typeMsb, jlong typeLsb, jlong uuidMsb, jlong uuidLsb,
        jint pf, jintArray arr) {
    if (!initLib(env))
        return -1235;
    ...
    DLSYM_OR_ELSE(libaudioclient,
                  ZN7android11AudioEffect19getEffectDescriptorEPK12audio_uuid_sS3_jP19effect_descriptor_s) {
        DLSYM_OR_RETURN(libaudioclient,
                        ZN7android11AudioEffect19getEffectDescriptorEPK12audio_uuid_sP19effect_descriptor_s,
                        -1235)
    }
    int ret;
